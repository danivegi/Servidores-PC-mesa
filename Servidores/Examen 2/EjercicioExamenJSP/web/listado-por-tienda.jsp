        ...
        <table width="100%" border="0">
            ...
            <tr align="center">
                <td colspan="7">
                    ...
                    <h3>Listado de ventas por tienda</h3>

                    <form method="post" action="rtdo-listado-tiendas.jsp">
                    <label>Tienda</label>
                    <select name="sel" id="sel">
                        <%
                            ArrayList<Tienda> listaTiendas = Bd.consultarTiendas();
                            for (int x = 0; x < listaTiendas.size(); x++) {%>
                            <option value="<%=listaTiendas.get(x).getIdtienda()%>">
                                <%=listaTiendas.get(x).getNombre()%> - <%=listaTiendas.get(x).getPoblacion()%>
                        </option>
                        <%}%>
                    </select><br></br>
